  ...
  methods: {
    async updateOrder(event) {
      ...
      try {
        ...
        Nova.success(this.__('NovaResourceClick.reorderSuccessful'));
      } catch (e) {
        if (e && e.response && e.response.data && e.response.data.canNotReorder) {
          const id = e.response.data.canNotReorder;
          Nova.error(this.__('NovaResourceClick.reorderNotAllowedFor', { id }));
          this.refreshResourcesList();
          return;
        }
        Nova.error(this.__('NovaResourceClick.reorderError'));
      }
    ...
    async moveToStart(resource) {
      ...
      try {
        ...
        Nova.success(this.__('NovaResourceClick.moveToStartSuccessful'));
      } catch (e) {
        Nova.error(this.__('NovaResourceClick.reorderError'));
      }
      this.reorderLoading = false;
    ...
    async moveToEnd(resource) {
      ...
      try {
        ...
        await this.refreshResourcesList();
        Nova.success(this.__('NovaResourceClick.moveToEndSuccessful'));
      } catch (e) {
        Nova.error(this.__('NovaResourceClick.reorderError'));
      }
      this.reorderLoading = false;
